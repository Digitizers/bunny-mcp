...
export function registerPullZoneTools(server, http, cache) {
  ...
  server.tool(
    ...
    async ({ page, per_page, search }) => {
      try {
        // No includeCertificate. The projector removes both certificate fields
        // either way, so the option could not produce its advertised result —
        // and asking for it pulled hostname PRIVATE KEYS across the wire into
        // this process to be discarded. Same removal as the video-library
        // access-key option.
