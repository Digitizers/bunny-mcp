...
/**
 * Shapes where EVERY string field is scrubbed, not just the ones I remembered.
 *
 * A log entry is about a request, so every piece of text in it may quote that
 * request. Scrubbing `Url` and the prose while leaving `Path` — the same target
 * under another name — was the fifth time this file patched one member of the
 * URL family and left a sibling standing. The SHAPE carries the rule now, so a
 * field added to a diagnostic later is covered on the day it is added.
 *
 * Populated below, once the shapes it names exist.
 */
const TEXT_SCRUBBED = new Set();
...
const PULL_ZONE = [
  ...
  // Caching, shielding, restriction and optimizer configuration — all named in
  // bunny_get_pull_zone's description or bunny_update_pull_zone's field list.
  // Booleans, numbers, region codes and scalar lists; nothing here has room for
  // a credential, and a zone you cannot read the caching rules of is not much
  // use for debugging one.
  "EnableOriginShield", "OriginShieldZoneCode", "OriginShieldMaxQueueWait",
  "OriginShieldQueueMaxWaitTime", "OriginShieldEnableConcurrencyLimit",
  "EnableQueryStringSort", "QueryStringVaryParameters",
  ["CacheErrorResponses", ["StatusCode", "CacheAge", "Enabled"]],
  "EnableLogging", "LoggingIPAnonymizationEnabled", "LogAnonymizationType",
  "AllowedReferrers", "BlockedReferrers", "BlockedIps", "BlockPostRequests",
  "ZoneSecurityIncludeHashRemoteIP", "VerifyOriginSSL", "FollowRedirects",
  "ConnectionLimitPerIPCount", "RequestLimit", "LimitRatePerSecond",
  "LimitRateAfter", "BurstSize", "OriginConnectTimeout", "OriginResponseTimeout",
  "OptimizerDesktopMaxWidth", "OptimizerMobileMaxWidth",
  "OptimizerImageQuality", "OptimizerMobileImageQuality",
  "OptimizerEnableWebP", "OptimizerEnableManipulationEngine",
  "OptimizerMinifyCSS", "OptimizerMinifyJavaScript", "OptimizerWatermarkEnabled",
  ["EdgeRules", EDGE_RULE],
...
// A transcoding failure names the fetch_url a video was imported from, and that
// URL is one an operator signs. The codes stay verbatim; the prose is scrubbed.
const TRANSCODE_NOTE = ["timeStamp", "level", "issueCode", ["message", scrubText], ["value", scrubText]];
...
const SAFE_MATCH_LOCATIONS = new Set([
  "REQUEST_METHOD", "REMOTE_ADDR", "GEO_COUNTRY_CODE",
  "REQUEST_PATH", "HOST", "REQUEST_PROTOCOL",
  // NOT REQUEST_URI: a URI includes its query, and `/callback?token=secret` is
  // a perfectly ordinary thing to write a rule against. It is handled below
  // instead — the path survives, the query does not.
]);

/** Locations whose operand is a URI: safe once the query is taken off it. */
const PATH_MATCH_LOCATIONS = new Set(["REQUEST_URI"]);

/**
 * `/callback?token=secret` -> `/callback`.
 *
 * For a field that IS a path — a rule's REQUEST_URI operand, a log entry's
 * `Path`. A path is structured enough to cut precisely, so it is reduced rather
 * than withheld; the token rule that governs PROSE has no such luxury.
 */
export function stripQuery(value) {
  return typeof value === "string" ? value.split(/[?#]/)[0] : value;
}

/** Whether two locations would be judged the same way. */
function sameVerdict(a, b) {
  const cls = (t) => {
    const u = String(t ?? "").toUpperCase();
    if (SAFE_MATCH_LOCATIONS.has(u)) return "safe";
    if (PATH_MATCH_LOCATIONS.has(u)) return "path";
    return "withheld";
  };
  return cls(a) === cls(b);
}
...
export function scrubMatchValues(values, type) {
  ...
  const where = String(type ?? "").toUpperCase();
  if (SAFE_MATCH_LOCATIONS.has(where)) return values;
  if (PATH_MATCH_LOCATIONS.has(where)) return values.map(stripQuery);
...
/** Registered after RATE_LIMIT_RULE is declared — see the note beside it. */
function registerRateLimitHook(shape) {
  OBJECT_HOOKS.set(shape, (out, src) => {
    if (!("value" in out)) return out;
    // A rate-limit rule can carry its operand at the TOP level as well as
    // inside `variables[]`, and the two say the same thing in different shapes.
    // The nested hook covered one of them; a token matched in an Authorization
    // header came back verbatim through the other. Judged by the same location
    // the variables name, and withheld outright when they name none — an
    // operand with no stated location is an operand nobody can vouch for.
    // EVERY location, not the first one. Reading `variables[0]` made the
    // verdict depend on array order: a safe REQUEST_PATH sitting in front of a
    // REQUEST_HEADERS published the operand on the strength of its neighbour.
    // A value whose location cannot be established unambiguously is withheld —
    // with no variables at all, with several disagreeing, or with any one of
    // them naming a place a credential can sit.
    const types = Array.isArray(src.variables) ? src.variables.map((v) => v?.type) : [];
    const unanimous = types.length > 0 && types.every((t) => sameVerdict(t, types[0]));
    const scrubbed = scrubMatchValues([src.value], unanimous ? types[0] : null);
    out.value = Array.isArray(scrubbed) ? scrubbed[0] : scrubbed;
    return out;
  });
}

// The dedicated bot-detection configuration response. Sharing SHIELD_ZONE meant
// the tool's own categories and thresholds were dropped — and, once a response
// carried only these fields, the no-fields-matched check turned a successful
// request into an error.
const BOT_DETECTION = [
  "shieldZoneId", "enabled", "mode", "sensitivity", "threshold",
  "categories", "allowedBots", "challengeType", "actionOnDetect",
];
...
const WAF_PROFILE = [
  // A WAF profile is a RESOURCE, not a status — the sibling rules response from
  // the very same tool goes through WAF_RULE, and this went through untouched.
  // The asymmetry was the tell: one half of one tool's security configuration
  // was allow-listed and the other half exempt.
  //
  // Named from what bunny_get_waf_rules advertises — managed rulesets and their
  // enablement — and from the sibling shape's vocabulary. It is not transcribed
  // from a live response, and that is survivable in this direction only: a field
  // Bunny returns that is missing here is dropped, and a shape that matches
  // nothing raises rather than returns. Both fail toward withholding.
  "id", "profileId", "name", "enabled", "shieldZoneId",
  "rulesetId", "rulesetType", "executionMode", "blockMode",
  "thresholdScore", "paranoiaLevel", "sensitivity",
  ["rules", WAF_RULE],
  // NOT: description and any free-text annotation. A profile description is
  // operator-authored control-plane text, which is where the DNS_RECORD comment
  // finding established a credential gets written down. Identity and enablement
  // say what the profile is without it.
  //
  // NOT: ruleConfiguration / actionParameters — excluded on the sibling shape
  // for the same reason, and excluded here so the rule is not taught in one
...
registerRateLimitHook(RATE_LIMIT_RULE);
...
// One origin failure. The URL is the point of the log — an operator debugging a
// 502 needs to know which request failed — so it is reduced rather than dropped,
// and the free-text reason is scrubbed for URLs quoted inside it.
const ORIGIN_ERROR = [
  // Url and Path are both request targets and both structured, so both are
  // REDUCED. The prose fields beside them are withheld instead — see scrubText.
  "Timestamp", "DateTime", ["Url", scrubUrl], ["Path", stripQuery], "StatusCode",
  "ErrorCode", ["Error", scrubText], "OriginIp", "EdgeIp", "PullZoneId",
  ["Message", scrubText], ["Reason", scrubText], "Count",
];

/**
 * The global search response.
 *
 * NOT a passthrough, and this one was not reported — it turned up sweeping every
 * PASSTHROUGH route for "does this answer with a RESOURCE?" after the publish
 * finding. `bunny_global_search` is a read tool, available in the default mode,
 * and it answers with pull zones, DNS zones, storage zones, video libraries and
 * edge scripts — every shape whose secrets the rest of this file exists to
 * remove, returned whole because the route said passthrough.
 *
 * Each container projects through the shape it already has. A container Bunny
 * adds later has no shape, so the structural rule drops it: fail closed, and
 * loudly if the whole payload turns out to be something else.
 */
const SEARCH = [
  ["PullZones", PULL_ZONE], ["pullZones", PULL_ZONE],
  ["DnsZones", DNS_ZONE], ["dnsZones", DNS_ZONE],
  ["StorageZones", STORAGE_ZONE], ["storageZones", STORAGE_ZONE],
  ["VideoLibraries", VIDEO_LIBRARY], ["videoLibraries", VIDEO_LIBRARY],
  ["EdgeScripts", EDGE_SCRIPT], ["edgeScripts", EDGE_SCRIPT],
];

TEXT_SCRUBBED.add(ORIGIN_ERROR);
TEXT_SCRUBBED.add(TRANSCODE_NOTE);
...
const ROUTES = [
  ...
  // fetch creates a video and answers with it.
  [/^\/library\/\d+\/videos\/fetch$/i, VIDEO],
  ...
  // Publish answers with the created RELEASE, carrying the operator's note —
  // the same note EDGE_SCRIPT_RELEASE drops from the history. A write that
  // returns a resource is a read wearing a different verb.
  [/^\/compute\/script\/\d+\/publish(\/[^/]+)?$/i, EDGE_SCRIPT_RELEASE],
  ...
  // Shield
  // Anchored to the one metrics path the shield tool fetches. Unanchored, a
  // `/shield/metrics/` prefix matched anything Bunny puts beneath it.
  [/^\/shield\/metrics\/overview\/[^/]+$/i, PASSTHROUGH],
  [/^\/shield\/waf\/(custom-rule|rules)(\/[^/]+)?$/i, WAF_RULE],
  [/^\/shield\/waf\/profiles(\/[^/]+)?$/i, WAF_PROFILE],
  ...
  [/^\/shield\/shield-zone\/[^/]+\/bot-detection$/i, BOT_DETECTION],
  ...
  // and this names those three. An earlier version matched `[a-z-]+` instead,
  // reasoning that an action added to the enum later would otherwise arrive
  // undeclared — which is the reason to pin it, not to widen it. Arriving
  // undeclared is what raises, and raising is how a new endpoint gets read
  // before its body is returned. As written, `[a-z-]+` also spelled every
  // OTHER path Bunny may hang off an app, credentials among them, and declared
  // them all safe in advance.
  [/^\/mc\/apps\/[^/]+\/(deploy|undeploy|restart)$/i, PASSTHROUGH],
  ...
  // Account & misc
  [/^\/user$/i, ACCOUNT],
  // Only `/billing` is fetched (bunny_get_billing_summary). This was
  // `(\/.*)?` — a subtree, which is a promise about endpoints nobody has
  // read, including the payment-method paths under it.
  ...
  // shape rather than a catch-all. Projected, not passed through: a log entry
  // quotes the request that failed, and that request is where a signed URL is.
  [/^\/\d+\/[0-9-]+$/i, ORIGIN_ERROR],
];
...
/**
 * Reduces any URL found INSIDE free text, leaving the rest of the text alone.
 *
 * Diagnostics are prose, and prose about a failed request quotes the request.
 * An origin error names the URL it could not fetch; a transcoding failure names
 * the `fetch_url` a video was imported from — and both of those URLs are the
 * ones an operator signs.
 *
 * This is deliberately not an allow-list, because prose has no vocabulary to
 * allow: the alternative is dropping the field, and a diagnostic that will not
 * say what failed is not worth returning. It is best-effort by construction —
 * it reduces what it recognises as a URL and cannot promise there is nothing
 * else in the sentence. Fields whose whole content is operator-authored (a DNS
 * `Comment`, a release `Note`, `CustomHTML`) are dropped rather than scrubbed
 * for exactly that reason; these two are kept because the tool exists to
 * report them.
 */
export function scrubText(value) {
  if (typeof value !== "string" || value === "") return value;
  // Split on whitespace and keep the separators, so the sentence reassembles
  // exactly.
  return value.split(/(\s+)/).map(scrubToken).join("");
}

/** What a withheld target is replaced with. */
const WITHHELD_TARGET = "(withheld: URL)";

/**
 * Whether a token carries a request target that may hold a credential.
 *
 * Deliberately crude, and that is the point. Nine findings were spent teaching a
 * reducer the shapes a URL can take in prose — a scheme, userinfo, no userinfo,
 * a bracketed IPv6 host, a colon in a password, an opaque query, a parenthesis
 * in a path, leading punctuation, a `key=` label. Every one was a shape the
 * parser had not met, and there was no reason to think the tenth did not exist:
 * prose is not a language you can parse.
 *
 * So nothing is parsed. A token either carries the marks of a credential-bearing
 * target or it does not:
 *
 *   - an `@` — userinfo, wherever and however it is written;
 *   - a `?` or `#` with material attached to it — a query or a fragment,
 *     whether or not a path precedes it.
 *
 * There is no interior to walk, so there is no shape to miss. A token that has
 * none of those is left exactly as written, which keeps `origin.example/file`,
 * `https://origin.example/ok`, `10s:` and `3.5` intact — a plain target
 * carries nothing, and saying which host failed is most of a diagnostic's value.
 *
 * What this does NOT catch, said plainly: a secret embedded in a path segment,
 * as in `/download/sk_live_x/file`. Nothing short of understanding the operator's
 * URL scheme would, and the previous reducer did not catch it either.
 */
function looksLikeTarget(token) {
  if (token.includes("@")) return true;
  // A `?` or `#` with content on BOTH sides of it. Requiring a `.` or a `/`
  // somewhere in the token was a proxy for "this is a URL", and it missed a
  // relative target — `callback?token=secret` has neither. What actually
  // separates a query from prose is that a query is ATTACHED: `why? because`
  // has a space after the mark, while `callback?token=secret` has something
  // on each side.
  if (/\S[?#]\S/.test(token)) return true;

  // A query- or fragment-ONLY reference — `?token=secret`, `#access_token=x`,
  // `?eyJhbGciOiJIUzI1NiJ9`, `#123456` — is a legal URI reference with no path
  // in front of it, so there is nothing before the delimiter to require. If a
  // token BEGINS with the mark and carries anything at all, it is a target.
  //
  // An earlier version exempted an all-digit fragment, so that a diagnostic
  // could still say `see section #3`. That exemption was a claim about what a
  // number MEANS, and a number means nothing on its own: `#123456` is a section
  // reference and a PIN and a session id, written identically. The version
  // before it asked whether the remainder was long enough to be a secret, which
  // is the same guess wearing a length. Both were wrong the same way, so the
  // question is retired rather than narrowed — `#3` is withheld along with
  // `#123456`, and losing a section number is the side to be wrong on.
  return /^[?#]\S/.test(token);
}

/**
 * Withholds one whitespace-delimited token if it carries a target.
 *
 * The whole token goes, punctuation and all. An earlier draft kept the
 * sentence's own brackets around the placeholder and produced
 * `((withheld: URL))`; there is nothing to preserve here, because the token is
 * being replaced rather than edited.
 */
function scrubToken(token) {
  return looksLikeTarget(token) ? WITHHELD_TARGET : token;
}
...
function pick(obj, fields) {
  ...
  for (const f of fields) {
    ...
    if (typeof nested === "function") {
      ...
      //
      // Declared over TEXT, and only text reaches one. Every scrubber returns a
      // non-string unchanged — they are string functions — so an `Error` or an
      // `OriginUrl` that arrived as an OBJECT would have been handed back whole,
      // walking straight past the rule that structure survives only through a
      // declared nested shape. A structured value here is dropped like any
      // other unvetted structure.
      if (typeof value === "string") out[name] = nested(value);
      continue;
    }
    if (nested) {
    ...
    if (isScalar(value) || isScalarArray(value)) {
      out[name] = TEXT_SCRUBBED.has(fields)
...
function projectValue(value, fields) {
  ...
  // A primitive where a SHAPE was expected. Returning it as-is treated "there
  // are no fields to pick" as "there is nothing here to hide", and those are
  // different claims: a double-encoded `/user` arrives as a JSON STRING with
  // the account's ApiKey inside it, and a string has no fields at all.
  //
  // Refused, for the same reason an undeclared path is. Every route reaching
  // here has a field shape; a payload the shape cannot be applied to is a
  // payload nobody has vetted.
  // `null` is the exception, and the only one: Bunny serialises an absent
  // optional block that way — `autoscaling: null` on an app that does not
  // autoscale, `wafEngineConfig: null` with WAF off. There are no contents to
  // vet, so refusing it would fail an entire valid listing over a field that
  // says "nothing here".
  if (value === null) return null;

  if (typeof value !== "object") {
    throw new Error(
      `Response projection expected an object and received ${typeof value}. ` +
      "A shape cannot vet a primitive — if this route legitimately answers with one, declare it " +
      "PASSTHROUGH or RAW in lib/project.js.",
    );
  }
