...
// BUNNY_API_KEY_FILE lets the key live in a mode-0600 file instead of client
// config, where every MCP client persists it in plain text. The env var wins
// when both are set, so existing setups keep working unchanged.
export function loadApiKey(env, readFile) {
  if (env.BUNNY_API_KEY) return env.BUNNY_API_KEY;
  if (env.BUNNY_API_KEY_FILE) {
    let raw;
    try {
      raw = readFile(env.BUNNY_API_KEY_FILE, "utf-8");
    } catch {
      return null;
    }
    const key = raw.trim();
