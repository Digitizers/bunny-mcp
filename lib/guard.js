...
export function installProjection(http, mode = "api", allowSource = false) {
  http.interceptors.response.use((res) => {
    ...
    if (mode === "storage") {
      // Storage is file content by definition; a listing is the JSON case
      // handled below.
      const stype = String(res.headers?.["content-type"] ?? "");
      if (stype && !stype.includes("json")) return res;
    } else {
      // A non-JSON body on the management API is not automatically a file. It
      // passes only where the route declares RAW, and a RAW route's body is
      // still the operator's to release — edge-script source carries hard-coded
      // keys as readily as any other operator-authored text.
      const type = String(res.headers?.["content-type"] ?? "");
      if (type && !type.includes("json")) {
        if (shapeFor(res.config?.url ?? "") !== RAW) {
          throw new Error(
            // normalisePath, never the raw url: this message travels to the
            // client through handleToolError, and a diagnostic is output like
            // any other. `bunny_purge_url` submits a signed URL as `?url=…`,
            // so the query is exactly where a token would be — and an
            // interceptor throw carries no `response`, so the rejection
        ...
        if (!allowSource) {
          const size = typeof res.data === "string" ? res.data.length : (res.data?.length ?? 0);
