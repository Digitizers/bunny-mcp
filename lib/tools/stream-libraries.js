...
export function registerStreamLibraryTools(server, http, cache) {
  ...
  server.tool(
    ...
    async ({ page, per_page, search }) => {
      try {
        // No includeAccessKey: the projector strips ApiKey/ReadOnlyApiKey either
        // way, so asking Bunny for them only moved credentials across the wire to
        // be discarded — and the option advertised a capability this server will
        // not provide.
  ...
  server.tool(
    ...
    async ({ id }) => {
      try {
        // See bunny_list_video_libraries: the access-key option is gone on purpose.
