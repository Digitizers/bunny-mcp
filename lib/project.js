...
export function scrubText(value) {
  ...
  return value
    ...
    .replace(/(?<=\S)[?#][\w.\-%[\]]+=[^\s"'<>)\]]*/g, "")
    // A schemeless `user:pass@host/path?token=…`. The WHOLE target is reduced,
    // not just the `user:pass@` in front of it: removing the part a credential
    // is known to sit in and keeping the rest is the deny-list mistake this
    // file has now made three times — userinfo on an origin, then its query,
    // then this. `scrubUrl` needs a scheme to parse, so one is lent for the
    // reduction and dropped again afterwards.
    .replace(
      /\b[\w.+-]+:[^\s:@/]+@[\w.-]+(?::\d+)?(?:\/[^\s"'<>)\]]*)?/g,
      (m) => scrubUrl(`https://${m}`).replace(/^https:\/\//, ""),
    );
}
