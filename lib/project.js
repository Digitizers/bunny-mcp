 ...
 *   - a `?` or `#` on something with a `.` or a `/` in it — a query or fragment
 *     on a host or a path.
 *
 ...
 * `https://origin.example/ok`, `10s:`, `#3` and `3.5` intact — a plain target
...
function looksLikeTarget(token) {
  ...
  // has a space after the mark and `#3` has nothing before it, while
  // `callback?token=secret` has something on each side.
  if (/\S[?#]\S/.test(token)) return true;

  // A query- or fragment-ONLY reference — `?token=secret`, `#access_token=x`,
  // `?eyJhbGciOiJIUzI1NiJ9` — is a legal URI reference with no path in front of
  // it, so there is nothing before the delimiter to require. What separates one
  // from a prose reference like `#3` is what comes AFTER: an assignment, a
  // separator, or simply more material than a section number ever has.
  const suffix = /^([?#])(\S*)$/.exec(token);
  if (!suffix) return false;

  // A `?`-first token is a query, always. The numeric exemption below exists
  // for prose references — `#3`, `#42` — and prose does not write `?123456`:
  // a question mark in a sentence is followed by a space, so anything attached
  // to one is a query string, and a numeric one is a PIN or a session id.
  if (suffix[1] === "?") return suffix[2] !== "";

  // Anything but a plain number. The first version of this asked whether the
  // remainder was long enough to be a token, which is a guess about the size of
  // a secret — `?abc123` is a perfectly good one and sits under any threshold.
  //
  // Turned around, the question has an answer: what legitimately follows a bare
  // `?` or `#` in a DIAGNOSTIC? A reference — `#3`, `#42`. Everything else is a
  // target. `#top` is withheld too, and that is the right side to be wrong on.
  return !/^\d+$/.test(suffix[2]);
}
