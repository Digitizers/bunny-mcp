...
import { projectResponse, projectStorageListing, projectErrorBody, shapeFor, RAW } from "./project.js";
    ...
    if (mode === "storage") {
      ...
      if (type && !type.includes("json")) {
        if (shapeFor(res.config?.url ?? "") !== RAW) {
          throw new Error(
            `A non-JSON response from "${res.config?.url ?? ""}" has no declared policy. ` +
