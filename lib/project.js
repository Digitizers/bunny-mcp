...
const PULL_ZONE = [
  "Id", "Name", "Type", "Enabled", "Suspended", ["OriginUrl", stripUrlUserinfo], "OriginType",
...
// A rule's match inputs. `matchValues` is operator-written, but it is the rule's
// substance — a WAF rule you cannot read the conditions of cannot be reviewed.
...
/**
 * Strips `user:password@` from a URL.
 *
 * A pull zone's origin is essential operational information and must come back
 * — but Bunny accepts an authenticated origin, and the credentials for it live
 * inside the URL. Dropping the field would hide where the zone points; keeping
 * it whole would publish the origin's password.
 */
export function stripUrlUserinfo(value) {
  ...
  try {
    ...
    // Not a URL we can parse. Anything with an `@` before the first `/` may be
    // carrying userinfo, so it is withheld rather than guessed at.
    return /^[^/]*@/.test(value.replace(/^[a-z]+:\/\//i, "")) ? "(withheld: unparseable URL with userinfo)" : value;
  }
}
