...
/** `/callback?token=secret` -> `/callback`. Operands here are paths, not URLs. */
...
const ORIGIN_ERROR = [
  "Timestamp", "DateTime", ["Url", scrubUrl], "Path", "StatusCode",
...
export function scrubText(value) {
  ...
  // exactly. WHITESPACE is the delimiter — see scrubToken for why.
  return value.split(/(\s+)/).map(scrubToken).join("");
}

/**
 * Reduces one whitespace-delimited token.
 *
 * Eight findings went into delimiting URLs inside prose with character classes:
 * a scheme, then userinfo, then no userinfo, then a bracketed IPv6 host, then a
 * colon in the password, then an opaque query, then a parenthesis in the path.
 * Every one was the same shape — a character the class did not expect — and
 * every fix taught the class one more character.
 *
 * A URL in prose has exactly one reliable boundary: whitespace. So the text is
 * split there and each token judged whole. Trailing punctuation is peeled off
 * first and put back after, which is what a character class was really trying
 * to approximate, and the middle of the token is then nobody's business:
 * parentheses, brackets, colons and anything else ride along inside it.
 */
function scrubToken(token) {
  if (!/[:@?#]/.test(token)) return token;

  // Punctuation is peeled off BOTH ends. Trailing alone was half the job: a URL
  // quoted in prose is as often wrapped — `(https://user:pass@host/f)` — and a
  // leading `(` defeated every anchored check inside, so the credentials rode
  // through untouched even with no query on the end.
  ...
  // A diagnostic often labels its target — `fetch_url=https://…` — and the
  // label made every anchored check below miss. Peeled off and put back, the
  // same way the punctuation around it is.
  //
  // Only `=`. A `:` label is indistinguishable from the first half of userinfo,
  // and peeling `origin:` off `origin:user:pass@host` splits the credential in
  // two so that neither half matches anything. The userinfo rule below handles
  // that shape whole, which is why it can.
  const labelled = /^([\w.+-]+=)(.+)$/s.exec(core);
  if (labelled && /^[a-z][a-z0-9+.-]*:\/\//i.test(labelled[2])) {
    return head + labelled[1] + scrubToken(labelled[2]) + tail;
  }
  if (labelled && /[./]/.test(labelled[2]) && /[@?#]/.test(labelled[2])) {
    return head + labelled[1] + scrubToken(labelled[2]) + tail;
  }

  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(core)) return head + scrubUrl(core) + tail;

  // Schemeless. Userinfo is credentials wherever it sits; a query or fragment
  // is one only when the token is a path or a host, which is what keeps
  // "section #3" and "why?" intact.
  // A protocol-relative `//host` keeps its slashes; the userinfo after them
  // goes. Anchored so the pattern cannot wander into the middle of a token.
  let out = core.replace(/^(\/\/)?[\w.+-]+:[^\s@/]+@/, "$1");
