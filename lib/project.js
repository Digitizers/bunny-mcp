...
export function scrubText(value) {
  if (typeof value !== "string" || value === "") return value;
  return value
    // `]` is allowed inside: an IPv6 host is written `[2001:db8::1]`, and
    // stopping at the bracket handed scrubUrl half a URL, which it correctly
    // refused to parse and withheld — safe, but unreadable.
    .replace(/\b[a-z][a-z0-9+.-]*:\/\/[^\s"'<>)]+/gi, (m) => scrubUrl(m))
    // A QUERY OR FRAGMENT, wherever it appears and whatever precedes it.
    //
    // Three rounds went into recognising URL shapes in prose — with a scheme,
    // then schemeless with userinfo, then schemeless with neither. Enumerating
    // shapes is the losing half of this: the thing worth removing is the query
    // construct itself, which is where tokens live and which is never
    // information an operator needs from a diagnostic. `?key=value` after a
    // non-space character goes, whether it hangs off `https://host/p`,
    // `//host/p`, `host/p`, or nothing recognisable at all.
    //
    // A FRAGMENT counts too, and is not an afterthought: OAuth's implicit flow
    // returns its token in one, so `#access_token=…` is a credential by
    // construction rather than by accident.
    .replace(/([^\s"'<>)\]]*[./][^\s"'<>)\]]*)[?#][^\s"'<>)\]]+/g, "$1")
    // USERINFO, as a construct — not as part of a host shape.
    //
    // Six findings went into recognising targets: with a scheme, schemeless
    // with userinfo, schemeless without, and finally a bracketed IPv6 host that
    // `[\w.-]+` could never match. Every one was the same mistake enumerating
    // URL shapes for queries had been, and it has the same answer: match the
    // CONSTRUCT and stop caring what follows it. `user:pass@` is credentials in
    // front of a hostname, an IPv4 literal, `[2001:db8::1]`, or something this
    // file has never seen. The query rule above removes the other half
    // independently, so the two compose without either parsing a host.
    .replace(/(?<![\w:])[\w.+-]+:[^\s@/]+@/g, "");
}
