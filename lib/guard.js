...
/**
 * Whether a downloaded body is really a Bunny storage listing.
 *
 * Deliberately narrow. The question being settled is not "is this JSON" — an
 * operator's stored `.json` file is JSON, and reducing it to `{}` was the bug
 * that put the download bypass here in the first place. It is "does this carry
 * the marks of a Bunny storage object", which a file would have to be written
 * specifically to imitate.
 *
 * And where the two genuinely cannot be told apart, this errs toward calling it
 * a listing, because the costs are not symmetrical: mistaking a listing for a
 * file returns account metadata past the allow-list, while mistaking a file for
 * a listing tells the operator to use the listing tool and returns nothing.
 */
const STORAGE_OBJECT_MARKERS = ["StorageZoneName", "Path", "ObjectName"];

function isStorageListing(body) {
  let text;
  if (typeof body === "string") text = body;
  else if (Buffer.isBuffer(body)) text = body.toString("utf-8");
  else if (body instanceof ArrayBuffer) text = Buffer.from(body).toString("utf-8");
  else if (ArrayBuffer.isView(body)) text = Buffer.from(body.buffer, body.byteOffset, body.byteLength).toString("utf-8");
  else return false;

  let parsed;
  try { parsed = JSON.parse(text); } catch { return false; }
  // An empty array is an empty directory, and carries nothing either way; it
  // is left to pass through rather than raised as an error on a real file that
  // happens to contain `[]`.
  ...
  return parsed.every((entry) =>
    ...
    && typeof entry === "object"
    && !Array.isArray(entry)
    && STORAGE_OBJECT_MARKERS.every((field) => Object.prototype.hasOwnProperty.call(entry, field)));
}
...
export function installProjection(http, mode = "api", allowSource = false) {
  http.interceptors.response.use((res) => {
    ...
    //
    // But asking is not receiving. Hand the download tool a DIRECTORY path and
    // the storage API answers that very request with a JSON listing, and this
    // bypass returned it whole — past the allow-list, unknown fields and all.
    // Content-type could not be trusted to catch it and neither could
    // responseType: each is a single signal, one claimed by the responder and
    // one by the caller, and a listing satisfies both. So the body is asked
    // what it IS. Only a body that is unmistakably a Bunny listing is treated
    // as one; anything else is the operator's bytes and passes through.
    if (String(res.config?.responseType ?? "") === "arraybuffer") {
      if (mode === "storage" && isStorageListing(res.data)) {
        throw new Error(
          `"${normalisePath(res.config?.url ?? "")}" is a directory, not a file. ` +
          "Use bunny_list_storage_files to list it.",
        );
      }
      return res;
    }
